...
class AgentConfigLoader:
    ...
    def _find_config_file(self) -> Optional[str]:
        """自动查找 config.yaml 文件，不存在时返回 None（纯环境变量模式）。"""
        current_dir = Path(__file__).parent
        for _ in range(3):
            config_path = current_dir / "config.yaml"
            if config_path.exists():
                logger.info("找到配置文件: %s", config_path)
                return str(config_path)
            current_dir = current_dir.parent
        logger.debug("未找到 config.yaml，将使用环境变量作为唯一配置来源")
        return None

    def _load_yaml_config(self, config_path: Optional[str]) -> dict:
        """加载 YAML 配置文件；config_path 为 None 时返回空字典（纯环境变量模式）。"""
        if config_path is None:
            return {}
        try:
            ...
            logger.warning("加载配置文件失败，将使用环境变量: %s", e)
            return {}
