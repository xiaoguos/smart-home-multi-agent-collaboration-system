...
本文围绕毕业设计项目 **MossAi** 展开研究与实现。MossAi 是一个基于 Python 和大语言模型的多智能体协同框架与示例系统，旨在通过统一的 Agent 抽象、标准化的工具调用接口以及可配置的任务流程编排，支持在不同业务领域快速构建智能应用。系统设计上，MossAi 将「任务」视为核心驱动单元，围绕任务流转建立了多智能体协作机制；工程实现上，则通过清晰的目录结构、配置驱动的 Agent 注册方式以及统一日志与错误处理策略，提升了系统的可维护性与可扩展性。

本文的主要工作与创新点如下：（1）提出并实现了一个面向多智能体协同的整体架构，将 LLM 能力封装为可复用的 Agent 组件，并通过任务调度器协调多 Agent 分工协作；（2）设计了一套统一的工具调用与环境交互接口，使智能体能以自然语言驱动底层具体操作，增强了系统对外部系统的集成能力；（3）以 `air_cleaner_agent` 为示例，构建了面向空气净化/环境控制场景的领域 Agent，展示了 MossAi 在现实场景中的适配与扩展方式；（4）通过一系列实验，对系统在任务完成率、交互体验、可扩展性等方面进行了评估，并分析了项目在工程实践和研究层面上的意义和不足。

实验结果表明，MossAi 能够在典型任务场景下有效完成多步任务规划与执行，相比传统脚本方案在灵活性和可维护性方面具有优势；同时，系统在引入新领域 Agent 时保持较低的开发成本，体现了良好的扩展性。最后，本文对 MossAi 的不足之处进行了分析，并展望了在多 Agent 协作策略、长期记忆机制、安全性和鲁棒性等方面的后续研究方向。

**关键词**：大语言模型；智能体（Agent）；多智能体系统；任务调度；工具调用；MossAi
...
This thesis presents **MossAi**, a multi-agent framework and demonstrative system built upon Python and LLMs. MossAi aims to provide a unified agent abstraction, standardized tool invocation interfaces, and configurable task orchestration mechanisms, thus supporting rapid construction of intelligent applications across different business domains. In terms of system design, MossAi treats “task” as the core driving unit and builds multi-agent collaboration mechanisms around task flows. In terms of engineering implementation, MossAi adopts a clear directory structure, configuration-driven agent registration, and unified logging and error-handling strategies to improve maintainability and extensibility.

The main contributions of this thesis are as follows: (1) proposing and implementing an overall architecture for multi-agent collaboration, where LLM capabilities are encapsulated into reusable agent components and coordinated by a task scheduler; (2) designing a unified interface for tool invocation and environment interaction, enabling agents to drive concrete operations through natural-language instructions and improving the ability to integrate with external systems; (3) building a domain-specific agent, `air_cleaner_agent`, for air purification and environmental control scenarios, demonstrating how MossAi can be adapted and extended to real-world applications; (4) designing experiments to evaluate task success rate, interaction experience, and extensibility, and analyzing the practical and research significance of the project.

Experimental results show that MossAi can effectively complete multi-step task planning and execution in typical scenarios, and exhibits advantages in flexibility and maintainability compared to traditional script-based solutions. Moreover, the cost of introducing new domain agents remains relatively low, which reflects good extensibility. Finally, this thesis discusses the limitations of MossAi and outlines future work on multi-agent collaboration strategies, long-term memory mechanisms, security, and robustness.

**Keywords**: Large Language Model; Agent; Multi-agent System; Task Scheduling; Tool Invocation; MossAi
...
本课题来源于对当前 LLM 应用形态和工程实践需求的观察，目标是在真实的工程环境下，以毕业设计项目的形式，构建一个具有代表性的多智能体系统——**MossAi**。该项目既作为一个可运行的智能应用原型，也作为具备教学与研究价值的开源框架雏形。
...
本文结构安排如下：第二章介绍与本课题相关的技术背景与理论基础；第三章给出 MossAi 的系统总体设计与架构；第四章深入描述关键模块的设计与实现；第五章以 `air_cleaner_agent` 为示例，展示领域 Agent 的具体设计与使用流程；第六章进行实验与评估；第七章对本文工作进行总结并提出未来研究方向。
...
在 MossAi 中，大语言模型被视为一个「能力核心」，负责自然语言理解和推理；围绕这一核心，通过 Agent 封装，将其能力与具体任务、工具调用结合，形成可工程化的智能系统。
...
在 MossAi 中，多 Agent 协作的思想主要体现在：将不同职责划分为不同的 Agent，通过任务调度器与消息机制，使它们在一个任务生命周期中分工合作。
...
MossAi 在设计中抽象了工具接口，使 Agent 不直接依赖某个具体工具的实现细节，而是以统一接口描述工具的名称、输入、输出和错误处理方式。这样，既可以方便地接入数据库查询、HTTP 请求、文件系统操作，也可以接入类似 `air_cleaner_agent` 这种与具体设备或业务逻辑相关的操作。

### 2.4 Python 工程化与项目结构设计

MossAi 以 Python 为主要开发语言，Python 在 AI 与数据处理领域有广泛生态支持。为了提高项目的可维护性与扩展性，良好的工程结构设计非常关键。
...
MossAi 项目遵循上述基本原则，并在此基础上进一步考虑了多 Agent 协作和领域 Agent 扩展的需求。
...
该机制为多 Agent 协作提供了基础，使 MossAi 能够在更复杂的任务场景中保持清晰的执行流程与可监控性。
...
为了统一管理不同类型的 Agent，MossAi 设计了一个通用的 Agent 抽象模型，通常包含以下核心要素：
...
调度器根据任务的类型和当前系统状态，选择合适的 Agent 与执行策略。简单场景下，调度器可以通过映射表（task_type → agent）实现；复杂场景下，可以引入规则引擎或由 LLM 自身生成调用序列。MossAi 在当前阶段偏向规则化和配置化的调度策略，以保证行为可控和易于调试。

### 4.3 工具调用与外部环境交互模块

工具模块的设计是连接 LLM 和真实世界的关键。在 MossAi 中，工具通常具有以下特征：
...
在多 Agent 协作和大量外部工具调用的场景中，日志与错误处理尤为重要。MossAi 采用集中式日志记录方案，将任务流转过程中的关键事件记录下来，包括：
...
`air_cleaner_agent` 是 MossAi 中一个具有代表性的领域智能体，主要面向空气净化与环境控制场景。其典型使用场景包括：
...
在 MossAi 的目录结构中，`air_cleaner_agent` 通常位于 `agents/air_cleaner_agent/` 目录下，主要包括：
...
尽管具体代码实现因项目版本而异，但整体思路是：将与空气净化相关的「知识」与「操作能力」封装在此 Agent 内部，使其成为 MossAi 框架中的一个可插拔角色。

### 5.3 与 MossAi 框架的集成方式

`air_cleaner_agent` 与 MossAi 的集成主要通过以下几个方面实现：
...
这个示例展示了 MossAi 在具体场景中的应用方式，也体现了多步骤任务在一个领域 Agent 中的闭环处理能力。
...
为了评估 MossAi 的效果与性能，本文设计了一系列实验场景，主要从任务完成率、效率、可扩展性和用户体验等方面进行考察。实验环境大致如下：
...
通过多次重复试验，统计成功完成任务的比例和平均交互轮数。实验表明，对于结构相对清晰、指令较为明确的任务，MossAi 能够在较高比例下成功完成；对于过于模糊或信息缺失较多的指令，系统会主动进行多轮询问，以提高成功率。
...
与传统的全手动操作流程相比，MossAi 在用户体验上有一定提升；与单 Agent、无明确任务建模的简单脚本方案相比，MossAi 通过更清晰的任务流转与多 Agent 分工，能够在复杂任务场景下保持更好的可控性与可维护性。

### 6.3 可扩展性与工程复用性分析

为了验证 MossAi 的可扩展性，我们尝试添加新的领域 Agent，例如一个负责简单信息检索或日报生成的 Agent。在不修改核心框架的前提下，只需要：

- 新建 Agent 模块，实现统一接口；  
- 在配置中注册该 Agent 与对应任务类型；  
- 在必要时添加新工具或复用现有工具。

实践表明，这一过程相对简单，说明 MossAi 在设计上基本达到了「通过扩展而非修改」的设计目标，有利于后续持续演进。

从工程复用性角度看，MossAi 中关于任务建模、调度、日志与错误处理的部分同样可以被其他 LLM 应用引用或借鉴，为后续项目提供基础框架。
...
本文围绕毕业设计项目 MossAi 展开研究与实践，完成了以下几方面的工作：

- 分析了当前 LLM 应用和多智能体系统的现状与问题，提出了 MossAi 的设计目标和原则；  
...
总体而言，MossAi 作为一个毕业设计项目，在系统架构、工程实现和实验评估方面都达到了预期目标，为后续进一步研究和开发打下了基础。

### 7.2 存在的不足

尽管 MossAi 已经具备一定功能和架构完整性，但仍存在一些不足之处：

- **多 Agent 协作策略较为简单**：目前主要采用规则化和配置化的调度策略，对更复杂的任务协作模式支持有限。  
- **长期记忆与知识管理不足**：系统尚未系统性引入长期记忆机制，无法对长期交互中的知识进行有效管理和复用。  
- **安全性与鲁棒性有待提升**：工具调用的安全策略、异常场景下的自恢复能力仍有提高空间。  
- **实验规模有限**：由于资源限制，实验规模和场景覆盖面相对有限，尚不足以得出更普适的结论。

这些不足既是当前工作的局限，也是未来改进的方向。

### 7.3 未来工作展望

未来可在以下几个方面对 MossAi 进行拓展和深入研究：
...
通过这些拓展，MossAi 有望从一个毕业设计项目逐步发展为更具实用价值与研究价值的多智能体平台。
