    ...
    async def stream(self, query, context_id) -> AsyncIterable[dict[str, Any]]:
        ...
        for item in self.graph.stream(inputs, config, stream_mode='values'):
            ...
            if (
                ...
                if any(name == 'set_lamp_power' for name in tool_names):
                    tip = '正在切换床头灯电源…'
                elif any(name == 'get_lamp_status' for name in tool_names):
                    tip = '正在查询床头灯状态…'
                elif any(name == 'set_lamp_brightness' for name in tool_names):
                    tip = '正在调节亮度…'
                elif any(name == 'set_lamp_color_temp' for name in tool_names):
                    tip = '正在设置色温…'
                elif any(name == 'set_lamp_color' for name in tool_names):
                    tip = '正在设置颜色…'
                elif any(name == 'set_lamp_scene' for name in tool_names):
                    tip = '正在应用场景模式…'
                else:
                    tip = '正在处理您的请求…'
                yield {
