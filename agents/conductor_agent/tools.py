...
# 静态兜底配置（仅在无法连接数据库时使用）
_FALLBACK_AGENTS: Dict[str, Dict[str, Any]] = {}


def _get_dynamic_agents() -> Dict[str, Dict[str, Any]]:
    """
    从数据库动态读取已注册的 Agent 列表。

    - 内嵌模式（backend 进程）：直接使用 backend 的 database.query
    - 独立模式（uv run .）：尝试 import，失败时返回静态兜底配置
    """
    try:
        from database import query  # noqa: PLC0415
        rows = query(
            """
            SELECT agent_code, agent_name, host, port, description
            FROM agent_config
            WHERE is_enabled = 1 AND agent_code != 'conductor'
            """
        )
        if not rows:
        ...
        for row in rows:
            code = str(row["agent_code"])
            host = str(row.get("host") or "localhost").strip()
            port = row.get("port")
            if not port:
                continue
            url = (
            ...
            agents[code] = {
                "name": str(row.get("agent_name") or code),
                "url": url,
        ...
        logger.debug("动态获取 Agent 列表失败，使用兜底配置: %s", exc)
        return _FALLBACK_AGENTS


# 向后兼容别名（工具内部统一调用 _get_dynamic_agents()，此变量仅保留供外部引用）
REGISTERED_AGENTS = _FALLBACK_AGENTS
...
def list_available_agents():
    """获取所有已注册的代理服务列表"""
    try:
        registered = _get_dynamic_agents()
        agents_info = []
        for agent_id, config in registered.items():
            agents_info.append({
                "id": agent_id,
                "name": config["name"],
                "url": config["url"],
...
def execute_agent_command(agent_id: str, command: str):
    ...
    try:
        ...
        agent_config = registered[agent_id]
...
def control_device(device_type: str, action: str, parameters: Dict[str, Any] = None):
    ...
    try:
        ...
        if device_type not in registered:
            execution_time = int((time.time() - start_time) * 1000)
            error_result = {
                "success": False,
                "message": f"设备类型 {device_type} 不支持",
                "error": f"设备类型 {device_type} 不支持",
                "supported_types": list(registered.keys()),
        ...
        agent_config = registered[device_type]
        agent_url = agent_config["url"]
        ...
        operation_record = create_device_operation_record(
            system_user_id=1000000001,
            device_type=device_type,
            device_name=registered.get(device_type, {}).get("name"),
            action=action,
            parameters=parameters,
            success=False,
            error_message=error_msg,
...
def query_data_mining_agent(query: str, user_id: int = 1000000001):
    ...
    """
    try:
        agent_config = _get_dynamic_agents().get("data_mining")
        if not agent_config:
            return json.dumps({
                "success": False,
